...
export default function Chat() {
    ...
    useEffect(() => {
        if (friend) {
            const fetchMessages = async () => {
                const token = localStorage.getItem("token");
                try {
                    const response = await axios.get(`/chat/${friend.id}`, {
                        headers: { Authorization: `Bearer ${token}` },
                    });
                    setMessages(response.data);
                    setUniqueChatId(response.data.length > 0 ? response.data[0].uniqChatTopic : `chat_${localStorage.getItem("userId")}_${friend.id}`);
                } catch (error) {
    ...
    const setupStompClient = () => {
        ...
        stompClient.current.connect({}, (frame) => {
            ...
            stompClient.current.subscribe(`/topic/messages/${uniqueChatId}`, (message) => {
    ...
    const sendMessage = () => {
        ...
        stompClient.current.send(`/app/chat/${uniqueChatId}`, {}, JSON.stringify(messageObj));
        setMessage("");
    };
                    ...
                    {friend && (
                        <>
                            <div className="h-15p flex rounded-t-md justify-between items-center px-4 bg-gray-300">
                                <img src={friend.photos[0] ?? logo} className="h-24" alt="friend" />
                            ...
                            <div className="flex-1 flex px-2 py-3 flex-col rounded-b-md gap-3 overflow-y-auto bg-aman">
                                {messages.map((msg) => (
                                    <div
                                        key={msg.id}
                                        className={`bg-white max-w-xl break-all p-2 flex flex-col items-end ${
                                            msg.sender.id === friend.id ? "justify-start mr-auto" : "justify-end ml-auto"
                                        }`}
                                    >
                                        <h2 className="text-2xl">{msg.messageText}</h2>
                                        <span className="text-sm">{time(new Date(msg.messageDate))}</span>
                                    </div>
                                ))}
                            </div>
