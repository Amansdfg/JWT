...
export default function Chat() {
    ...
    return (
        <section className="w-screen h-dvh flex flex-col dark:bg-darkMode">
            <Header/>
            <div className="py-3 px-2 flex-1 md:overflow-hidden flex gap-4">
                <div className={`flex-col gap-3 flex ${!params.id ? " w-full md:w-30p" : " w-30p hidden md:flex"}`}>
                  <Friends friends={user.friends}/>
                </div>
                <div className={`px-4 py-3 bg-aman dark:bg-lightMode drop-shadow-md rounded-md  ${!params.id?"hidden w-70p md:flex":"w-full md:w-70p"}`}>
